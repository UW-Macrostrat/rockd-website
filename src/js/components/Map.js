...
class Map extends Component {
  ...
  componentDidMount() {
    ...
    this.map.on("click", (event) => {
      ...
      let points = this.map.queryRenderedFeatures(event.point, {
        layers: ["points"],
      });
      ...
      if (this.state.showBedrock) {
        let burwellFeatures = this.map.queryRenderedFeatures(event.point, {
          layers: ["burwell_fill"],
        });
  ...
  queryMap(lngLat, z) {
    ...
    let url = `${
      Constants.MACROSTRAT_API_URL
    }/mobile/map_query_v2?lng=${lngLat.lng.toFixed(5)}&lat=${lngLat.lat.toFixed(
      5
    )}&z=${parseInt(z)}`;
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      nextProps.activeStop[0] != this.props.activeStop[0] &&
      nextProps.activeStop[1] != this.props.activeStop[1]
    ) {
      this.map.flyTo({
    ...
    return (
      <div className="map-container2">
        <Button
          ...
          size="small"
          aria-label="bedrock"
          className={showBedrock ? "bedrock-control active" : "bedrock-control"}
          onClick={() => {
            this.toggleBedrock();
          }}
        >
            ...
            {Object.keys(mapInfo).length ? (
              <div className="map-source-attrs">
                ...
                {mapInfo.age && mapInfo.age.length ? (
                  <div className="map-source-attr">
                    <span className="attr">Age: </span> {mapInfo.age} (
                    {mapInfo.b_int.b_age} - {mapInfo.t_int.t_age}
