 ...
 * {@link FilterURLBinding}. Sorts are handled generically (`?sort=-rating,near`).
 *
 ...
 *
 * Ported from the `web` repo's `~/components/data-view-url-state`. The two
 * copies should converge — either into a shared package or into the data-sheet
 * library itself, which is where its own notes put a first-class URL sync.
 */
...
import { locationAtom } from "./url-atoms";
 ...
 * which is why this maps state ⇄ a *record* rather than a single value.
 */
 ...
 * *created*, so a linked view issues one correct request; the sync component
 * then has nothing to do on mount and takes over for later changes and
 * back/forward.
 */
...
/** `[{key: "rating", ascending: false}]` → `"-rating"`. */
