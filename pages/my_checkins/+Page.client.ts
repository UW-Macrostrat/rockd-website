...
export function Page() {
  ...
  if (showSettings) {
    overlay = h(
      Sidebar,
      { title: "Settings", onClose: () => setSettings(false) },
      h(ContextPanel)
    );
    ...
    overlay = h(
      Sidebar,
      { title: "My checkins", showCloseButton: false },
      h(CheckinList, { setInspectPosition, personId: rockdPersonId })
    );
  }
...
function useMapStyle(type, mapboxToken, showSatellite, showOverlay, personId) {
  ...
  // Scope the checkin tiles to this user
  ...
  const overlayStyle = showOverlay
    ? mergeStyles(_macrostratStyle, userOverlayStyle)
    : userOverlayStyle;
