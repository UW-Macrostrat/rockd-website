...
The development workflow here is closely tied to that of the Macrostrat web interface (https://github.com/UW-Macrostrat/web).
...
- Most `@macrostrat/*` packages are built from the `https://github.com/UW-Macrostrat/web-components` monorepo. Source can be inspected from there, or via a local clone of the repository. If you are working locally, ask where the source is if you are not sure.
- **`@macrostrat/*` packages resolve to their published dist**, NOT local sibling source. Editing web-components source has **no effect** on this app's dev server until that package is released and the version bumped here. `vite.config.ts` marks these `noExternal` + `dedupe`, so they're bundled from the cached dist.
- If you do have access to the `UW-Macrostrat/web-components` library, work within the installed package's current API, but feel free to propose library changes that can be landed as a separate release. Do not edit web-components without asking permission.

## State management

- **Use `jotai`'s atomic approach for state management** and add helper libraries as appropriate for common patterns
